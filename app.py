...
COMMON_PASSWORD = settings['password']
...
def login():
    if request.method == 'POST':
        ...
        # Check if the matriculation number is 6 digits and password is correct
        if len(matriculation_number) == 6 and matriculation_number.isdigit() and password == COMMON_PASSWORD:
            session['logged_in'] = True
            session['matriculation_number'] = matriculation_number
...
def index():
    ...
    if request.method == 'POST':
        ...
        if 'matriculation_number' in session:
            database = "app.db"
            matriculation_number = session['matriculation_number']
            conn = create_connection(database)
            insert_interaction(conn, matriculation_number, original_question, reply)
